...
public class LerTXT_Tres {
    ...
    public static void LeituraTXT() throws FileNotFoundException, IOException {
        ...
        BufferedReader br = new BufferedReader(new FileReader("setsT2.txt"));
        String txt;
    ...
    public static void PreparaTerceiro() throws FileNotFoundException, IOException {
        ArrayList<String> conjunto3 = new ArrayList<>();
        BufferedReader br = new BufferedReader(new FileReader("setsT3.txt"));
        String txt;
        while ((txt = br.readLine()) != null) {
            System.out.println(txt);
            if (Character.isLetter(txt.charAt(0))) {
                String parte5 = txt.substring(txt.indexOf("{") + 1, txt.indexOf("}"));
                String parte6[] = parte5.split(Pattern.quote(", "));
                for (String item : parte6) {
                    //System.out.print(item);
                    conjunto3.add(item);
                }
            } else {
        ...
        String[] conjAux3 = conjunto3.toArray(new String[conjunto3.size()]);
        int[] aux3 = new int[conjAux3.length];
        for (int i = 0; i < conjAux3.length; i++) {
            aux3[i] = Integer.parseInt(conjAux3[i]);
        }
